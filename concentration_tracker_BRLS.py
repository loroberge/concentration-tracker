...
class ConcentrationTrackerBRLS(Component):
    ...
    **Required Software Citation(s) Specific to this Component**
    ...
    def concentration(self, dt):
        ...
        # Get arrays of all nodes that underwent erosion and deposition
        erode_nodes_all = mg.core_nodes[mg.at_node["landslide__erosion"][mg.core_nodes]>0]
        deposit_nodes_all = mg.core_nodes[mg.at_node["landslide__deposition"][mg.core_nodes]>0]

        # Associate erode and deposit nodes with their critical node ID
        for crit_node in critical_nodes:
            erode_nodes[crit_node] = erode_nodes_all[nodes that flow to crit_node] # flow receiver is critical node
            deposit_nodes[crit_node] = erode_nodes_all[nodes that flow from crit_node]  # is flow receiver of critical node
       
        for crit_node in critical_nodes:
            while deposit_nodes_all > 0:
                1. FIND WHICH NODES IN deposit_nodes_all ARE RECEIVERS OF THE crit_node
                2. ADD THOSE NODES TO deposit_nodes[crit_node]
                    deposit_nodes[crit_node].append()???
                3. REMOVE THOSE NODES FROM deposit_nodes_all
                4. FIND WHICH NODES IN deposit_nodes_all ARE RECEIVERS OF THE deleted nodes
                5. REPEAT UNTIL EMPTY
                
            while erode_nodes_all > 0:
                1. FIND WHICH NODES IN erode_nodes_all HAVE THE crit_node AS A RECEIVER 
                2. ADD THOSE NODES TO erode_nodes[crit_node]
                    erode_nodes[crit_node].append()???
                3. REMOVE THOSE NODES FROM erode_nodes_all
                4. FIND WHICH NODES IN erode_nodes_all HAVE THE deleted nodes AS A RECEIVER
                5. REPEAT UNTIL EMPTY
