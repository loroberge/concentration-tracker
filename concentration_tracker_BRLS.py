    ...
    def _concentration_eroded(self, dt):
        ...
        for crit_node in self._critical_nodes:
            ...
            neighbors = np.concatenate(
                (
                    self._grid.active_adjacent_nodes_at_node[crit_node],
                    self._grid.diagonal_adjacent_nodes_at_node[crit_node],
                )
            )
            neighbors[self._grid.status_at_node[neighbors]==4]=-1            
            ...
            dist_to_erode_nodes = distances[crit_node,self._erode_nodes_all]            
            slope_to_erode_nodes = (
                topo[self._erode_nodes_all] - topo[crit_node]
                ) / dist_to_erode_nodes
            
            # Associate all erode_nodes lying at sliding_angle to crit_node
            slide_nodes_mask = np.isclose(slope_to_erode_nodes,sliding_angle)
            
            #erode_nodes[crit_node].append(crit_node)
            erode_nodes[crit_node].extend(self._erode_nodes_all[slide_nodes_mask])
            
            nodes_assigned.extend(self._erode_nodes_all[slide_nodes_mask]) 

            # Calculate eroded concentrations and volumes for each landslide
            CV_br_slide[crit_node] = np.sum(CV_br[erode_nodes[crit_node]])
            CV_sed_slide[crit_node] = np.sum(self._CV_sed[erode_nodes[crit_node]])
            V_eroded[crit_node] = np.sum(V_br[erode_nodes[crit_node]]+self._V_sed[erode_nodes[crit_node]])
            
        # Check that all nodes were assigned
        if not set(self._erode_nodes_all).issubset(nodes_assigned):
            raise Exception(str("Not all eroded nodes are assigned to a landslide."))
            
        if not np.sum(self._ls.landslides_volume)==np.sum(V_br[nodes_assigned]+self._V_sed[nodes_assigned]):
            raise Exception(str("Volumes eroded do not match with values from LS instance."))
            
        # Calculate concentration per unit volume eroded to each critical node
        # (to be passed through node and sent to deposits)
        self._C_eroded[crit_node] = (CV_br_slide[crit_node] + CV_sed_slide[crit_node]) / V_eroded[crit_node]
