# -*- coding: utf-8 -*-
"""
Created on Tue May 23 14:26:22 2023
...
soil_production_maximum_rate = 0.0000000000000000000000000000001
...
_ = mg.add_zeros('Sm__concentration', at='node', units= ['kg/m^3','kg/m^3'])
mg.at_node['Sm__concentration'] += 0#C_initial
mg.at_node['Sm__concentration'][ncols + int(3*ncols/4)-1] += C_initial
...
plt.show()
imshow_grid(mg, "Sm__concentration", cmap=cmap_Sm, color_for_closed='pink')
plt.show()
...
# Instantiating Concentration calculator will store the old value of H
H_old = mg.at_node["soil__depth"]
...
def calc_concentration(dt, H_old):
    
    # Define soil depth at latest timestep
    H_new = mg.at_node["soil__depth"]
    C_old = mg.at_node['Sm__concentration']
    ...
    mg.at_link['QC'] = mg.at_link['soil__flux']*mg.at_link['C']
    mg.at_link['QC'][np.isnan(mg.at_link['QC'])] = 0
    
    # Calculate flux concentration divergence
    dQCdx = mg.calc_flux_div_at_node(mg.at_link['QC'])
    
    # Run calculation
    C_local = C_old * (H_old/H_new)
    Production = (dt*P/2) * (H_old/H_new + 1)
    Decay = (dt*D/2) * (H_old/H_new + 1)
    
    # Replace nan values with zeros
    # (the intention is to remove problems with dividing by zero soil depth)
    # (obviously this method messes with any other situation where nans appear)
    C_local[np.isnan(C_local)] = 0
    Production[np.isnan(Production)] = 0
    Decay[np.isnan(Decay)] = 0

    mg.at_node['Sm__concentration'] = (C_local + (dt/H_new) * (- dQCdx) +
                                       Production - Decay
                                       )
    # Define H_old (for next step) as current soil depth
    H_old = H_new
...
def plot_hill_profile():
    ...
    C_soil = mg.at_node['Sm__concentration'][core_ids]
...
#    mg.at_node['bedrock__elevation'][mg.core_nodes] += uplift_per_step
    ...
    calc_concentration(dt=dt, H_old=H_old)
    ...
    record_M_total_nodes[i] = np.sum(mg.at_node['Sm__concentration'][mg.core_nodes] * 
